...
function KarenPlatePLATE_NUM() {
  const { currentTheme } = useTheme();
  const { currentTopic } = useTopic();
  const { karenheader } = KarenData();
  const { karenPhotosPlates } = KarenPhotosData();
  const plateData = karenPhotosPlates.find((plate) => plate.plate === "platePLATE_NUM");
...
export default KarenPlatePLATE_NUM;
`;
...
(async () => {
  for (let i = 1; i <= 53; i++) {
    ...
    const filePath = path.join(__dirname, `KarenPlate${plateNum}.jsx`);
    await fs.writeFile(filePath, content); // Use await here
    console.log(`Generated KarenPlate${plateNum}.jsx`);
  }

  console.log("All plate components generated!");
})();

// cd ~/Documents/memories/src/pages/Karen/photos
// node genKarenPhotoPlates.js
